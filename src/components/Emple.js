...
import EmpleForm from "./EmpleForm";
import EmpleTabla from "./EmpleTabla";
 ...
 const Emple = () => {
  ...
  const handleShow = () => setShow(true);
    ...
    const createUserInFirestore = (e) => {
      db.collection('Empleados').doc(e.documento).get()
  .then((docSnapshot) => {
    ...
    else
    {
      notify("Registro Exitoso")
       db.collection("Empleados").doc(e.documento).set(e);
    }
  });
    }
     return(
        ...
        <Modal.Body style={{width :"max-content"}}><EmpleForm hid={handleClose}  crea={createUserInFirestore}/></Modal.Body>
